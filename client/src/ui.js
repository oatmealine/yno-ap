...
function createConnectForm() {
  ...
  connectButton.addEventListener('click', async () => {
    ...
    creds.host = hostRow.querySelector('input').value;
    creds.password = passwordRow.querySelector('input').value;
    creds.slot = slotRow.querySelector('input').value;

    connectButton.setAttribute('disabled', 'true');
    try {
      await connect(creds.host, creds.password, creds.slot);
      creds.autoConnect = true;
      saveGlobal();
    } catch (err) {
      console.error(err);
